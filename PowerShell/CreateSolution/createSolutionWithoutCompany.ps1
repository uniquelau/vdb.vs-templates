...
Write-Host "*" -ForegroundColor Yellow
Write-Host "*" -ForegroundColor Yellow -nonewline; Write-Host " Solution name: " -nonewline; Write-Host "$PROJECT" -ForegroundColor DarkGreen
Write-Host "*" -ForegroundColor Yellow
Write-Host "**********************************************************" -ForegroundColor Yellow
...
foreach ($sc in Get-ChildItem -Path $TARGETDIR -Filter $Find -Recurse | where { test-path $_.fullname -pathtype leaf} ) {
	...
	Get-ChildItem $sc.FullName | foreach { 
		rename-item $_ $_.Name.Replace('_company_._project_', "${PROJECT}") 
	}
...
if ($folders) {
    ...
    foreach ($folder in $folders) {
        ...
        Rename-Item $folder.FullName $folder.Name.Replace('_company_._project_', "${PROJECT}")
    } 
}
...
$_Company = New-Object PsObject -Property @{ OldValue = '_company_.' ; NewValue = "" }
...
$ReplacementList += $_Company, $_Project, $_GUID1, $_GUID2, $_GUID3, $_GUID4, $_GUID5, $_GUID6
